	...
	<div class="row">
			...
			<div class="row">
				...
				<div class="kco-box col-xs-12 {if $giftAllowed==1}col-md-4{else}col-md-8{/if}">
					<form action="{$link->getModuleLink('klarnaofficial', $controllername, [], true)|escape:'html':'UTF-8'}" method="post" id="klarnamessage">
						...
						<div class="kco-target" {if !$message.message}style="display: none;"{/if}>
							<p id="messagearea">
		...
		<div class="kco-box col-xs-12 col-md-4">
            ...
            <ul class="kco-sel-list has-tooltips">
                ...
                <li class="kco-sel-list__item {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}selected{/if}">
                	<input class="kco-sel-list__item__radio" type="radio" name="delivery_option[{$id_address|intval}]" onchange="$('#klarnacarrier').submit()" id="delivery_option_{$id_address|intval}_{$option@index|intval}" value="{$key|escape:'html':'UTF-8'}" {if isset($delivery_option[$id_address|intval]) && $delivery_option[$id_address|intval] == $key}checked="checked"{/if} />
                    <label for="delivery_option_{$id_address|intval}_{$option@index|escape:'html':'UTF-8'}" class="kco-sel-list__item__label">

...
<section id="main">
    <section class="card">
        <div class="card-block">
            <div class="row">
                <div class="col-md-12">
                    <h3 class="h1 card-title">{l s='Pay by invoice or part payment' mod='klarnaofficial'}</h3>
                    <p>
                            ...
                            {foreach from=$kpm_pclasses item=kpm_plan}
                                {if $kpm_plan.pclass_id==-1 && !$invoiceIsdisabled}
                                    ...
                                    {if $kpm_plan.pclass_id==-1 && $klarna_invoice_fee>0}
                                        <label class="kpm_description_full" for="{$kpm_plan.pclass_id|escape:'htmlall':'UTF-8'}">{l s='Invoice fee' mod='klarnaofficial'} {$klarna_invoice_fee|escape:'htmlall':'UTF-8'}</label>
                                    {/if}
                                ...
                                {else}
                                    ...
                                    <div id="kpm_description_full_{$kpm_plan.pclass_id|escape:'htmlall':'UTF-8'}" class="kpm_description_full kpmtohide" for="{$kpm_plan.pclass_id|escape:'htmlall':'UTF-8'}">
                                    <table class="klarnapclasstable">
                                    {foreach from=$kpm_plan.details item=detail}
                                        {if $detail.label != 'N/A'}
                                            <tr><td>{$detail.label|escape:'htmlall':'UTF-8'}</td><td>
                                            {if isset($detail.value)}
                                            {$detail.value|escape:'htmlall':'UTF-8'} {/if}{if isset($detail.symbol)}{$detail.symbol|escape:'htmlall':'UTF-8'}{/if}</td></tr>
                                        {/if}
                                    {/foreach}
                                    </table>
                                    {if isset($kpm_plan.use_case)}{$kpm_plan.use_case|escape:'htmlall':'UTF-8'}{/if}
                                    </div>
                                    <br />
                                {/if}
                                        ...
                                        <label for="kpm_ssn" class="required">{l s='Social security number' mod='klarnaofficial'}</label>
                                            <div class="input-group">
                                              ...
                                              <span class="input-group-btn">
                                                <button class="btn btn-secondary button-getaddress" type="button" onclick="javascript:kpmfetchaddress($('#kpm_ssn').val());" id="kpmgetaddress" name="getaddress">{l s='Fetch' mod='klarnaofficial'}</button>
                                              </span>
                                              {/if}
                                            </div>
                                    </div>
...
<section id="main">
    <section class="kpm_card_center">
        <div class="card-block">
            <div class="row">
                <div class="col-md-12">
                    <p>
                        <a class="btn btn-default" href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}"><i class="icon-chevron-left"></i>{l s='Other payment methods' mod='klarnaofficial'}</a>
                            <button name="confirmkpm" id="confirmkpm" class="btn btn-primary add-to-cart{if $kpm_iso_code=='de' || $kpm_iso_code=='at'} hidden{/if}" type="submit" onclick="javascript:$('#confirmkpm').hide();">

...
function kpmfetchaddress(ssn)
{
	$.ajax({
		type: 'POST',
		url: kpm_getaddress_url,
		async: false,
		cache: false,
		dataType: 'json',
		data: 'ssn='+ssn+'&kpm_md5key='+hex_md5(ssn+kpm_md5key),
		success: function(jsonData)
		{
			if (jsonData.hasError)
			{
				$("#kpmresult").show();
				$("#kpmresult").html(jsonData.error);
				$("#confirmkpm").hide();
			}
			else
			{
				...
				$.each(jsonData, function(i, item) {
					...
					{
						kpm_i=1;
						if(jsonData[i].iscompany)
						{
							$("#kpm_streetname").val(jsonData[i].address);
							$("#kpm_coname").val(jsonData[i].address2);
							$("#kpm_company").val(jsonData[i].company);
							$("#kpm_zipcode").val(jsonData[i].zip);
							$("#kpm_city").val(jsonData[i].city);
							$("#kpm_country").val(jsonData[i].country);
						}
						else
						{
							$("#kpm_firstname").val(jsonData[i].firstname);
							$("#kpm_lastname").val(jsonData[i].lastname);
							$("#kpm_streetname").val(jsonData[i].address);
							$("#kpm_coname").val(jsonData[i].address2);
							$("#kpm_company").val('');
							$("#kpm_zipcode").val(jsonData[i].zip);

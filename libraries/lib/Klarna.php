...
class Klarna
{
     ...
     * @var string
     */
    protected $VERSION = 'prestaworks:php:api:3.2.0';
    ...
    public function config(
        ...
        $mode = self::LIVE, $pcStorage = 'json', $pcURI = 'pclasses.json',
        $ssl = true
    ) {
        try {
    ...
    public function getAddresses(
        ...
        foreach ($result as $tmpAddr) {
            try {
                ...
                if ($type === KlarnaFlags::GA_GIVEN) {
                ...
                } elseif ($type === KlarnaFlags::GA_LAST) {
                    ...
                    $addr->setCity($tmpAddr[3]);
                    $addr->setCountry($tmpAddr[4]);
                } elseif ($type === KlarnaFlags::GA_ALL) {
    ...
    public function addTransaction(
        ...
        } elseif (!($this->billing instanceof KlarnaAddr)
            && ($this->shipping instanceof KlarnaAddr)
        ) {
     ...
     *
     * @param string $rno   Reservation number
     * @param bool   $clear clear set data aftre updating. Defaulted to true.
     *
    ...
    public function updateChargeAmount($invNo, $type, $newAmount)
    {
        ...
        if ($type === KlarnaFlags::IS_SHIPMENT) {
            $type = 1;
        } elseif ($type === KlarnaFlags::IS_HANDLING) {
            $type = 2;
        }
    ...
    private function _fetchPClasses($storage, $country, $language, $currency)
    {
        ...
        foreach ($result as &$pclass) {
            ...
            $pclass[1] = self::num_htmlentities($pclass[1]);
        ...
        if (!function_exists('pcCmp')) {
            ...
            function pcCmp($a, $b)
            {
                if ($a->getDescription() == null
                    ...
                    return 0;
                } elseif ($a->getDescription() == null) {
                    return 1;
                } elseif ($b->getDescription() == null) {
                    return -1;
                } elseif ($b->getType() === 2 && $a->getType() !== 2) {
                    return 1;
                } elseif ($b->getType() !== 2 && $a->getType() === 2) {
                    return -1;
                }
